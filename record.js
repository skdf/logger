...
function X()
{
	...
	// ziskanie api kluca z parametra §apikey§ z externeho skriptu
	var scripts = document.getElementsByTagName('script');
	var myScript = scripts[ scripts.length - 1 ];
	var queryString = myScript.src.replace(/^[^\?]+\??/,'');
	apiKey = parseQuery( queryString ).apikey;
    ...
    //  jQuery(document).on('touchstart touchend touchmove touchleave touchcancel', function (event) { x.eventTouchReceived(event); });
